-- CRITICAL SECURITY FIX: Fix running data exposure
-- Drop the overly permissive policy that allows anyone to view all runs
DROP POLICY IF EXISTS "Users can view all runs" ON public.runs;

-- Create a secure policy that only allows users to view their own runs
